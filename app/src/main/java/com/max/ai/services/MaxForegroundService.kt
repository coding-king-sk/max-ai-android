...
import androidx.core.app.NotificationCompat; import com.max.ai.MaxApplication; import com.max.ai.R
import javax.inject.Inject; import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.*
@AndroidEntryPoint class MaxForegroundService : Service() {
    ...
    private fun startForeground() {
        ...
        val n = NotificationCompat.Builder(this, MaxApplication.CHANNEL).setContentTitle(getString(R.string.foreground_notification_title)).setContentText(getString(R.string.foreground_notification_text)).setSmallIcon(android.R.drawable.ic_btn_speak_now).setContentIntent(pi).setOngoing(true).setPriority(NotificationCompat.PRIORITY_LOW).build()
        startForeground(1, n)
    }
    ...
    private fun processCommand(text: String) { scope.launch { try { val r = agent.process(text); if (r.toolCalls.isNotEmpty()) agent.executeCalls(r.toolCalls) } catch (e: Exception) { android.util.Log.e("Max", e.message.toString()) } } }
